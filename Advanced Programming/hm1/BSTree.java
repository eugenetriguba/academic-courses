...
public class BSTree<T> implements Cloneable, Serializable, Iterable<T>, MyCollection<T>
{
	...
	{				  
        BSTNode<T> node = this.root;
	   	...
	   	while (node != null)
	   	{
            int result = ((Comparable) item).compareTo(node.value);
	...
	public boolean add(T item)
	{
		...
		{
			parent = node;
			result = ((Comparable) item).compareTo(node.value);
	...
	protected class BSTreeIterator implements Iterator<T>
	{
		...
		{
			this.nextNode = BSTree.this.root;		//line 88
		...
		public T next()
		{
			...
			{
				BSTNode<T> parent = nextNode.parent;
